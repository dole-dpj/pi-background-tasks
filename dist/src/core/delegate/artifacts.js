...
export class DelegateArtifactStore {
    ...
    totalSpilledBytes = 0;
    constructor(rootAbs, rootDisplay, now, manifest) {
    ...
    static async create(options) {
        ...
        const store = new DelegateArtifactStore(rootAbs, rootDisplay, options.now ?? (() => new Date()), manifest);
    ...
    async commitResult(pkg) {
        const ref = await this.write(DELEGATE_ARTIFACT_NAMES.result, serializeDelegateResultPackage(pkg));
    ...
    async readCommittedResult() {
        try {
            ...
            throw new DelegateError(`delegate result package could not be read at ${join(this.rootDisplay, DELEGATE_ARTIFACT_NAMES.result)}; no committed answer is available (${error instanceof Error ? error.message : String(error)})`, {
    ...
    async writeError(state, message) {
        await this.write(DELEGATE_ARTIFACT_NAMES.error, `${canonicalJson({ state, error: message })}\n`);
        await this.setState(state, message);
    }
    async setState(state, error) {
    ...
    async spillToolPayload(input) {
        ...
        if (nextTotal > input.maxTotalBytes) {
            throw new DelegateError(`delegate aggregate tool output would reach ${String(nextTotal)} bytes, exceeding the ${String(input.maxTotalBytes)}-byte cap for one run`, {
        ...
        const name = `t${String(input.turnSequence).padStart(4, '0')}-c${String(input.sourceCallIndex).padStart(4, '0')}-${safeCallId}.bin`;
        ...
        try {
            ...
            throw new DelegateError(`delegate could not durably spill a ${String(input.payload.length)}-byte tool result; the payload is not forwarded and no receipt is emitted`, {
     ...
     */
    async readSpillRange(relativePath, offset, length) {
        ...
        try {
            ...
            throw this.readFailure(relativePath, `cannot open artifact: ${error instanceof Error ? error.message : String(error)}`);
        }
        try {
            const stats = await handle.stat();
            const totalBytes = stats.size;
            if (offset + length > totalBytes) {
                throw this.readFailure(relativePath, `requested bytes ${String(offset)}..${String(offset + length)} exceed the artifact size of ${String(totalBytes)} bytes; the read is refused rather than silently shortened`);
            }
            const buffer = Buffer.alloc(length);
            const { bytesRead } = await handle.read(buffer, 0, length, offset);
            if (bytesRead !== length) {
                throw this.readFailure(relativePath, `read returned ${String(bytesRead)} of ${String(length)} requested bytes`);
            }
    ...
    readFailure(relativePath, reason) {
    ...
    async durableReplace(absPath, data) {
